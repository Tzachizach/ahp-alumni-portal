...
export default function ProfessorYoungPage() {
  ...
  const filtered = all.filter((a) => {
    ...
    return (
      ...
      a.favoriteMemory.toLowerCase().includes(q)
    );
  });
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 bg-scarlet/10 rounded-lg flex items-center justify-center">
            <Heart size={20} className="text-scarlet" />
          </div>
          <h1 className="text-2xl font-bold text-ohio-gray-dark">Memories of Professor Young</h1>
        </div>
        <p className="text-ohio-gray text-sm ml-13 pl-0.5">
          A collection of favorite memories shared by AHP alumni.
        </p>
      </div>
          ...
          {filtered.map((alumni) => (
            <div key={alumni.id} className="bg-white rounded-xl border border-ohio-gray-medium shadow-sm p-6">
              ...
              <blockquote className="text-ohio-gray-dark text-sm leading-relaxed italic mb-5 border-l-4 border-scarlet pl-4">
                &ldquo;{alumni.favoriteMemory}&rdquo;
              </blockquote>
